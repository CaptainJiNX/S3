...
function objectCopy(authInfo, request, sourceBucket,
    ...
    return async.waterfall([
        function checkSourceAuthorization(next) {
            return services.metadataValidateAuthorization(valGetParams,
                (err, sourceBucketMD, sourceObjMD) => {
                    ...
                    if (headerValResult.error) {
                        log.debug('does not pass conditional copy header test');
                        return next(headerValResult.error);
                    }
                    const storeMetadataParams =
        ...
        function getCipherBundle(storeMetadataParams, dataLocator, destBucketMD,
            destObjMD, next) {
            ...
            return kms.createCipherBundle(
                ...
                log, (err, cipherBundle) => {
                    ...
                    return next(null, storeMetadataParams, dataLocator,
                        destObjMD, cipherBundle);
                });
        },
        function goGetData(storeMetadataParams, dataLocator,
            destObjMD, cipherBundle, next) {
            ...
            return async.mapLimit(dataLocator, 1,
                ...
                function copyPart(part, cb) {
                    return data.get(part, log, (err, stream) => {
                        ...
                        return data.put(cipherBundle, stream, part.size,
        ...
        function storeNewMetadata(storeMetadataParams, destDataGetInfoArr,
            ...
            return services.metadataStoreObject(destBucketName,
                destDataGetInfoArr,
                cipherBundle, storeMetadataParams, err => {
                    if (err) {
                    ...
                    return next(null, storeMetadataParams, cipherBundle);
                });
        },
    ], (err, storeMetadataParams, cipherBundle) => {
        if (err) {
            return callback(err);
        }
        const xml = [
