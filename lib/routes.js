...
export default function routes(req, res, logger) {
    ...
    if (req.bucketName === 'healthcheck') {
        if (utils.isValidBucketName(req.bucketName) === false) {
            return routesUtils.responseNoBody(null, null, res, 500, log);
        }

        return routesUtils.responseXMLBody(null,
            '<healthcheck><status>OK</status></healthcheck>',
            res, log);
    }
